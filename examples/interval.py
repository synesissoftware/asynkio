...
async def run_asyncio_sleep(
    ...
    while True:
        ...
        d.log(
            sev.NOTICE,
            f"{label} slept for {_delta_as_ns(t1, t2):,}; ∆={_clipped_delta_as_ns(t0, t2, timeout_ms * 1_000_000)}",
        )
...
async def heartbeat(
    ...
    while True:
        ...
        d.log(
            sev.INFO,
            f"❤️  #{count} : {t2 - t0}, {t2 - t1}, {Duration.from_nanos((t2 - t0).as_nanos() % 1_000_000_000)}",
        )
